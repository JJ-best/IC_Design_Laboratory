...
module enigma_part4(clk, srst_n, load, encrypt, crypt_mode, table_idx, code_in, code_out, code_valid);
input clk;         // clock 
...
generate
	for (m = 31; m >= 0; m = m - 1) begin : GEN_PLUG_IN
	  assign  plug_xor_i[m] = ~|(code_in ^ plugboard_reg[m]);
	end
endgenerate
genvar l;
generate
	for (l = 31; l >= 0; l = l - 1) begin : GEN_PLUG_OUT
	  assign  plug_xor_o[l] = ~|(plug_inverse_i ^ plugboard_reg[l]);
	end
endgenerate
...
onehot2binary_32 encoder32_i(
	...
	.valid_o(plug_valid_i)
);
assign plug_pair_i = plug_mux_sel_i ^ 5'b00001;
assign plug_forward_o = (plug_valid_i)? plugboard_reg[plug_pair_i] : code_in;

onehot2binary_32 encoder32_o(
	.onehot_i(plug_xor_o),
	.binary_o(plug_mux_sel_o),
	.valid_o(plug_valid_o)
);
assign plug_pair_o = plug_mux_sel_o ^ 5'b00001; // this may be optimize by other method
assign code_out = (plug_valid_o)? plugboard_reg[plug_pair_o] : plug_inverse_i;
// ----- plugboard ----- //
...
assign RotorA_forward_o = link[plug_forward_o];
...
RotorB_cell U63(
	...
	.shift_i(code_in),
	.load_i(load), .encrypt_i(encrypt), .clk(clk),
	.shift_value_i(shift_value_b), .table_idx(table_idx),
...
assign shift_value_b = (crypt_mode == 1'b0)? RotorB_forward_o[1:0] : RotorB_inverse_i[1:0];
