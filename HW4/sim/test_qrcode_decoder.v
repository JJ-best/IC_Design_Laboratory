...
reg srst_n;
reg start;
wire [16-1:0] sram_rdata;

wire [10-1:0] sram_raddr;
wire [6:0] loc_y;
wire [6:0] loc_x;
...
// SRAM instantiation
sram_1024x16b  sram_1024x16b_U0(
	.clk(clk),
	.csb(1'b0),
	.wsb(1'b1),
	.wdata(1'b0), 
...
reg [6:0] golden_loc_y [0:`TOTAL_QRCode-1];
reg [6:0] golden_loc_x [0:`TOTAL_QRCode-1];
...
reg [6:0] this_golden_loc_y [0:`MAX_QRCode-1];
reg [6:0] this_golden_loc_x [0:`MAX_QRCode-1];
...
initial begin
	...
	for(i=`PAT_L; i<=`PAT_U; i=i+1) begin
		...
		sram_1024x16b_U0.bmp2sram(i, `RANK);  // load pattern (.bmp) into SRAM
		//sram_1024x16b_U0.display_sram;  // display the content inside SRAM
...
task read_golden;
	...
	begin
		...
		for(i=0; i<`TOTAL_QRCode; i=i+1) begin
			golden_loc_y[i] = temp_loc[i][14:8];
			golden_loc_x[i] = temp_loc[i][6:0];
		end
		...
		for(i=0; i<`NUM_PAT; i=i+1) begin
			for(j=0; j<num_qrcode_in_pat[i]; j=j+1) begin
				...
				char_in = $fgetc(text_file_in);  // skip newline at the end of a text pattern (0D0A)
				char_in = $fgetc(text_file_in);
				//$write("%h", char_in);
				cnt = cnt + 1;
			end
...
task display_golden;
	...
	begin
		...
		for(i=0; i<`NUM_PAT; i=i+1) begin
			...
			for(j=0; j<num_qrcode_in_pat[i]; j=j+1) begin
				$write("    ");
				$write("(%3d, %3d)  ", golden_loc_y[cnt], golden_loc_x[cnt]);
