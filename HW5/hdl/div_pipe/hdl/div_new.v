...
// Stage0~5  : wide path (M bits)
// Stage6~23 : narrow path (24 bits) for area saving
...
reg [MAX_WIDTH-1:0] divisor_n   [6:PIPE_DEPTH];
reg [MAX_WIDTH-1:0] remainder_n [6:PIPE_DEPTH];
reg [MAX_WIDTH-1:0] dividend_n  [6:PIPE_DEPTH];

// merchant keeps 24 bits
...
reg [MAX_WIDTH:0]  stage_sub_n [6:PIPE_DEPTH];
...
always @(posedge clk) begin
    ...
    dividend_w[0]  <= dividend[MAX_WIDTH-1:0]; // keep low 24 bits for later use
    mode_sel_t[0]  <= mode_sel;
    if (!stage_sub_w[0][M]) begin
        merchant_t[0]  <= {merchant_t[0][MAX_WIDTH-1:1], 1'b1};
...
// ---------------- stage 6 ---------------- (enter narrow path)
always @(*) begin
    divisor_n[6]   = divisor_w[5][MAX_WIDTH-1:0];
    dividend_n[6]  = dividend_w[5];
    mode_sel_t[6]  = mode_sel_t[5];
    stage_sub_n[6] = {{remainder_w[5][MAX_WIDTH-1:0], dividend_n[6][MAX_WIDTH-7]}} - {1'b0, divisor_n[6]};
end
always @(*) begin
    if (mode_sel_t[5]) begin
        merchant_t[6]  = merchant_t[5];
        remainder_n[6] = remainder_w[5][MAX_WIDTH-1:0];
    end else if (!stage_sub_n[6][MAX_WIDTH]) begin
        merchant_t[6]  = {merchant_t[5][MAX_WIDTH-2:0], 1'b1};
        remainder_n[6] = stage_sub_n[6][MAX_WIDTH-1:0];
    end else begin
        merchant_t[6]  = {merchant_t[5][MAX_WIDTH-2:0], 1'b0};
        remainder_n[6] = {remainder_w[5][MAX_WIDTH-2:0], dividend_n[6][MAX_WIDTH-7]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[7][MAX_WIDTH]) begin
        merchant_t[7]  = {merchant_t[6][MAX_WIDTH-2:0], 1'b1};
        remainder_n[7] = stage_sub_n[7][MAX_WIDTH-1:0];
    end else begin
        merchant_t[7]  = {merchant_t[6][MAX_WIDTH-2:0], 1'b0};
        remainder_n[7] = {remainder_n[6][MAX_WIDTH-2:0], dividend_n[6][MAX_WIDTH-8]};
    end
end
...
always @(posedge clk) begin
    ...
    end else if (!stage_sub_n[8][MAX_WIDTH]) begin
        merchant_t[8]  <= {merchant_t[7][MAX_WIDTH-2:0], 1'b1};
        remainder_n[8] <= stage_sub_n[8][MAX_WIDTH-1:0];
    end else begin
        merchant_t[8]  <= {merchant_t[7][MAX_WIDTH-2:0], 1'b0};
        remainder_n[8] <= {remainder_n[7][MAX_WIDTH-2:0], dividend_n[7][MAX_WIDTH-9]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[9][MAX_WIDTH]) begin
        merchant_t[9]  = {merchant_t[8][MAX_WIDTH-2:0], 1'b1};
        remainder_n[9] = stage_sub_n[9][MAX_WIDTH-1:0];
    end else begin
        merchant_t[9]  = {merchant_t[8][MAX_WIDTH-2:0], 1'b0};
        remainder_n[9] = {remainder_n[8][MAX_WIDTH-2:0], dividend_n[8][MAX_WIDTH-10]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[10][MAX_WIDTH]) begin
        merchant_t[10]  = {merchant_t[9][MAX_WIDTH-2:0], 1'b1};
        remainder_n[10] = stage_sub_n[10][MAX_WIDTH-1:0];
    end else begin
        merchant_t[10]  = {merchant_t[9][MAX_WIDTH-2:0], 1'b0};
        remainder_n[10] = {remainder_n[9][MAX_WIDTH-2:0], dividend_n[9][MAX_WIDTH-11]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[11][MAX_WIDTH]) begin
        merchant_t[11]  = {merchant_t[10][MAX_WIDTH-2:0], 1'b1};
        remainder_n[11] = stage_sub_n[11][MAX_WIDTH-1:0];
    end else begin
        merchant_t[11]  = {merchant_t[10][MAX_WIDTH-2:0], 1'b0};
        remainder_n[11] = {remainder_n[10][MAX_WIDTH-2:0], dividend_n[10][MAX_WIDTH-12]};
    end
end
...
always @(posedge clk) begin
    ...
    end else if (!stage_sub_n[12][MAX_WIDTH]) begin
        merchant_t[12]  <= {merchant_t[11][MAX_WIDTH-2:0], 1'b1};
        remainder_n[12] <= stage_sub_n[12][MAX_WIDTH-1:0];
    end else begin
        merchant_t[12]  <= {merchant_t[11][MAX_WIDTH-2:0], 1'b0};
        remainder_n[12] <= {remainder_n[11][MAX_WIDTH-2:0], dividend_n[11][MAX_WIDTH-13]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[13][MAX_WIDTH]) begin
        merchant_t[13]  = {merchant_t[12][MAX_WIDTH-2:0], 1'b1};
        remainder_n[13] = stage_sub_n[13][MAX_WIDTH-1:0];
    end else begin
        merchant_t[13]  = {merchant_t[12][MAX_WIDTH-2:0], 1'b0};
        remainder_n[13] = {remainder_n[12][MAX_WIDTH-2:0], dividend_n[12][MAX_WIDTH-14]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[14][MAX_WIDTH]) begin
        merchant_t[14]  = {merchant_t[13][MAX_WIDTH-2:0], 1'b1};
        remainder_n[14] = stage_sub_n[14][MAX_WIDTH-1:0];
    end else begin
        merchant_t[14]  = {merchant_t[13][MAX_WIDTH-2:0], 1'b0};
        remainder_n[14] = {remainder_n[13][MAX_WIDTH-2:0], dividend_n[13][MAX_WIDTH-15]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[15][MAX_WIDTH]) begin
        merchant_t[15]  = {merchant_t[14][MAX_WIDTH-2:0], 1'b1};
        remainder_n[15] = stage_sub_n[15][MAX_WIDTH-1:0];
    end else begin
        merchant_t[15]  = {merchant_t[14][MAX_WIDTH-2:0], 1'b0};
        remainder_n[15] = {remainder_n[14][MAX_WIDTH-2:0], dividend_n[14][MAX_WIDTH-16]};
    end
end
...
always @(posedge clk) begin
    ...
    end else if (!stage_sub_n[16][MAX_WIDTH]) begin
        merchant_t[16]  <= {merchant_t[15][MAX_WIDTH-2:0], 1'b1};
        remainder_n[16] <= stage_sub_n[16][MAX_WIDTH-1:0];
    end else begin
        merchant_t[16]  <= {merchant_t[15][MAX_WIDTH-2:0], 1'b0};
        remainder_n[16] <= {remainder_n[15][MAX_WIDTH-2:0], dividend_n[15][MAX_WIDTH-17]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[17][MAX_WIDTH]) begin
        merchant_t[17]  = {merchant_t[16][MAX_WIDTH-2:0], 1'b1};
        remainder_n[17] = stage_sub_n[17][MAX_WIDTH-1:0];
    end else begin
        merchant_t[17]  = {merchant_t[16][MAX_WIDTH-2:0], 1'b0};
        remainder_n[17] = {remainder_n[16][MAX_WIDTH-2:0], dividend_n[16][MAX_WIDTH-18]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[18][MAX_WIDTH]) begin
        merchant_t[18]  = {merchant_t[17][MAX_WIDTH-2:0], 1'b1};
        remainder_n[18] = stage_sub_n[18][MAX_WIDTH-1:0];
    end else begin
        merchant_t[18]  = {merchant_t[17][MAX_WIDTH-2:0], 1'b0};
        remainder_n[18] = {remainder_n[17][MAX_WIDTH-2:0], dividend_n[17][MAX_WIDTH-19]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[19][MAX_WIDTH]) begin
        merchant_t[19]  = {merchant_t[18][MAX_WIDTH-2:0], 1'b1};
        remainder_n[19] = stage_sub_n[19][MAX_WIDTH-1:0];
    end else begin
        merchant_t[19]  = {merchant_t[18][MAX_WIDTH-2:0], 1'b0};
        remainder_n[19] = {remainder_n[18][MAX_WIDTH-2:0], dividend_n[18][MAX_WIDTH-20]};
    end
end
...
always @(posedge clk) begin
    ...
    end else if (!stage_sub_n[20][MAX_WIDTH]) begin
        merchant_t[20]  <= {merchant_t[19][MAX_WIDTH-2:0], 1'b1};
        remainder_n[20] <= stage_sub_n[20][MAX_WIDTH-1:0];
    end else begin
        merchant_t[20]  <= {merchant_t[19][MAX_WIDTH-2:0], 1'b0};
        remainder_n[20] <= {remainder_n[19][MAX_WIDTH-2:0], dividend_n[19][MAX_WIDTH-21]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[21][MAX_WIDTH]) begin
        merchant_t[21]  = {merchant_t[20][MAX_WIDTH-2:0], 1'b1};
        remainder_n[21] = stage_sub_n[21][MAX_WIDTH-1:0];
    end else begin
        merchant_t[21]  = {merchant_t[20][MAX_WIDTH-2:0], 1'b0};
        remainder_n[21] = {remainder_n[20][MAX_WIDTH-2:0], dividend_n[20][MAX_WIDTH-22]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[22][MAX_WIDTH]) begin
        merchant_t[22]  = {merchant_t[21][MAX_WIDTH-2:0], 1'b1};
        remainder_n[22] = stage_sub_n[22][MAX_WIDTH-1:0];
    end else begin
        merchant_t[22]  = {merchant_t[21][MAX_WIDTH-2:0], 1'b0};
        remainder_n[22] = {remainder_n[21][MAX_WIDTH-2:0], dividend_n[21][MAX_WIDTH-23]};
    end
end
...
always @(*) begin
    ...
    end else if (!stage_sub_n[23][MAX_WIDTH]) begin
        merchant_t[23]  = {merchant_t[22][MAX_WIDTH-2:0], 1'b1};
        remainder_n[23] = stage_sub_n[23][MAX_WIDTH-1:0];
    end else begin
        merchant_t[23]  = {merchant_t[22][MAX_WIDTH-2:0], 1'b0};
        remainder_n[23] = {remainder_n[22][MAX_WIDTH-2:0], dividend_n[22][MAX_WIDTH-24]};
    end
end

assign merchant  = {{(N-MAX_WIDTH){1'b0}}, merchant_t[PIPE_DEPTH]};
assign remainder = {{(M-MAX_WIDTH){1'b0}}, remainder_n[PIPE_DEPTH]};
