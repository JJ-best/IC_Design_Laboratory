...
initial begin
  ...
  wait(u_steganography.u_SRAM.enable === 1 && u_steganography.u_SRAM.r_w === 1); // wait your setting
  #(CYCLE);
  @(negedge clk);
  ...
  if(error == 0) begin
    $display("%d ns: Successfully!! First 3 rows of data in SRAM are same!", $time);
    $finish;
  end
  else begin
